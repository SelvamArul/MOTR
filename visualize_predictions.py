...
from engine import visualize_predictions_mot
...
def main(args):
    ...
    if args.distributed:
        import sys
        sys.exit("Distributed Mode enabled: This should not happen")
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            print ('*'*30)
            print ("Loading mode ", args.resume)
        ...
        import sys
        sys.exit("args.resume not provided")



    if args.dataset_file in ['e2e_mot', 'mot', 'ori_mot', 'e2e_static_mot', 'e2e_joint', 'ycbv']:
        # train_func = train_one_epoch_mot
        dataset_val.set_epoch(1)
    output_dir.mkdir(exist_ok=True)
    
    visualize_predictions_mot(model, data_loader_val, device) 



if __name__ == '__main__':
    parser = argparse.ArgumentParser('MOTR visualize prediction script', parents=[get_args_parser()])
